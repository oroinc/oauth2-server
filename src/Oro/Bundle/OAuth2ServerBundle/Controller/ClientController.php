...
class ClientController extends AbstractController
{
    ...
    public function createAction(Request $request, string $type): array|Response
    {
        ...
        if ($response instanceof RedirectResponse) {
            ...
            if (\in_array(Client::CLIENT_CREDENTIALS, $entity->getGrants())) {
                $accessTokenData = $this->container->get(AccessTokenClient::class)->getAccessToken([
                    ...
                    'grant_type' => Client::CLIENT_CREDENTIALS
                ]);
    ...
    private function checkClientApplicableForType(Client $client, string $type): void
    {
        ...
        if ((!$client->isFrontend() && !$this->featureToggleChecker->isFeatureEnabled('user_login_password'))
            || ($client->isFrontend()
                && !$this->featureToggleChecker->isFeatureEnabled('customer_user_login_password')
            )
        ) {
