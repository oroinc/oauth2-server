...
class BackendMetadataTest extends WebTestCase
{
    #[\Override]
    ...
    public function testTryToGetMetadataViaDirectUrl(): void
    {
        $this->client->request(
            'GET',
            $this->getUrl('oro_oauth2_server_metadata'),
            [],
            [],
    ...
    public function testGetMetadataViaWellKnownAlias(): void
    {
        $this->client->request(
            ...
            $this->getUrl(
                ...
                ['metadataPath' => ltrim($this->getUrl('oro_oauth2_server_metadata'), '/')]
            ),
            [],
            [],
            ['Accept' => 'application/json']
        ...
        self::assertEquals(
            [
                'issuer' => $this->getUrl('oro_oauth2_server_metadata', [], true),
                'authorization_endpoint' => $this->getUrl('oro_oauth2_server_authenticate', [], true),
    ...
    {
        $backendPrefix = self::getContainer()->getParameterBag()->has('web_backend_prefix')
            ? ltrim(self::getContainer()->getParameter('web_backend_prefix'), '/')
            : '';
        if ($backendPrefix) {
            $backendPrefix .= '/';
        }
        ...
        $this->client->request(
            ...
            $this->getUrl(
                'oro_oauth2_server_metadata_well_known_alias',
                ['metadataPath' => $backendPrefix . 'unknown']
            ),
            [],
            [],
            ['Accept' => 'application/json']
